...
typedef enum {
 ...
 // Following struct is used to keep track of the current state
...
// Returns 1 if the given character is valid, 0 otherwise.
...
void DFA_Alpha(LexerState* lexerState)
{
    ...
    //   If not, tokenize as ident.
     ...
     // Place characters into buffer as we iterate through the word until we
     // see a special symbol or until a lexErr is invoked.
     while(!isEndOfWord(lexerState->sourceCode[lexerState->charInd]))
     {
       // Symbol is too long.
       ...
       buffer[i++] = lexerState->sourceCode[lexerState->charInd++];
     }

     // End of word.
     buffer[i] = '\0';

     int tokenId = checkReservedTokens(buffer);

     // Token was not found. It is an identifier.
     if (tokenId == -1)
     {
       tokenId = 2;
     }
     ...
     // Add token to our token list.
     addToken(&lexerState->tokenList, tok);
}
...
void DFA_Digit(LexerState* lexerState)
{
    ...
    char buffer[MAX_NUM_DIGIT_LENGTH + 1];
    ...
    {
      // Number is too long.
      if (i >= MAX_NUM_DIGIT_LENGTH)
      {
        lexerState->lexerError = NUM_TOO_LONG;
        return;
      }
      // Ill-formed variable starting with a digit.
      if (isalpha(lexerState->sourceCode[lexerState->charInd]))
      {
...
void DFA_Special(LexerState* lexerState)
{
    ...
    // We have encountered a comment.
    if (lexerState->sourceCode[lexerState->charInd] == '/'
      && lexerState->sourceCode[lexerState->charInd + 1] == '*')
    {
      lexerState->charInd += 2;

      // Skip over everything until end of the comment is reached.
      while (1)
      {
        if (lexerState->sourceCode[lexerState->charInd] == '*'
          && lexerState->sourceCode[lexerState->charInd + 1] == '/')
    ...
    int i = 0;
		...
		// The only two character special tokens are "<=", ">=", "<>", and ":=".

		if (!isSpecialSymbol(lexerState->sourceCode[lexerState->charInd]))
		{
			return;
		}

    while (isSpecialSymbol(lexerState->sourceCode[lexerState->charInd]))
    {
      if (i > 1)
      {
				break;
			...
			if (i > 0 && lexerState->sourceCode[lexerState->charInd] == ';')
			{
				break;
			}
    ...
    int tokenId = checkReservedTokens(buffer);
		...
		// We went one character too far. A case like this would occur if there was
		// a closing parenthesis followed by a semi-colon: ");".
    if (tokenId == -1)
    {
      buffer[1] = '\0';
			lexerState->charInd--;

			tokenId = checkReservedTokens(buffer);
    }
...
LexerOut lexicalAnalyzer(char* sourceCode)
{
    ...
    // Create & initialize lexer state
    LexerState lexerState;
    initLexerState(&lexerState, sourceCode);

    // While not end of file, and, there is no lexer error, continue lexing.
